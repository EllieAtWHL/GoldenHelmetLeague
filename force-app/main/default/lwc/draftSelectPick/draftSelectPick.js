...
export default class DraftSelectPick extends LightningElement {
    ...
    switch (valueChanged) {
      ...
      case "price":
        if (this.winnerBudget < event.detail.value) {
          event.target.setCustomValidity("Price exceeds remaining budget");
  ...
  handleConfirm() {
    this.isLoaded = true;
    const teamId = this.isSnake
      ? this.currentPick.teamId
    ...
    makePick({ pickMade: pick, playerName: this.playerSelectedName })
      ...
      .catch((error) => {
        ...
        if (
          errorMessage ===
          "Duplicate pick detected. This pick has already been made."
        ) {
