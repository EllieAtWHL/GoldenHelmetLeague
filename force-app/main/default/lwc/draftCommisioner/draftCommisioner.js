...
export default class DraftCommisioner extends LightningElement {
  ...
  get message() {
    return this.isSnake
      ? `On the clock: </br> ${this.currentPick.pickTeam}`
      : `Now nominating: </br> ${this.currentPick.pickTeam}`;
  }
  ...
  get currentPick() {
    let getPick = this.currentRound?.picks.filter(
      (pick) => pick.roundPickNumber === this.currentPickNumber
    );
  ...
  get nextPick() {
    let getPick = this.currentRound?.picks.filter(
      (pick) => pick.roundPickNumber === this.currentPickNumber + 1
    );
    return getPick ? getPick[0] : null;
  }
  ...
  get currentOverallPick() {
    let overallPick =
      ...
      (this.currentRoundNumber * this.teamTotal - this.teamTotal);
    return overallPick;
  }

  get previousPicks() {
    let picks = [];
    let previousRounds = this.draft?.rounds.filter(
      (round) => round.roundNumber < this.currentRoundNumber
    );
    previousRounds?.forEach((round) => {
      if (round.reverse) round.picks.reverse();
      picks = picks.concat(round.picks);
    });
    let thisRoundPicks = this.currentRound?.picks.filter(
      (pick) => pick.roundPickNumber < this.currentPickNumber
    );
    ...
    tempDraft.rounds.forEach((round) => {
      round.picks.forEach((pick) => {
        if (!this.currentPickNumber && !pick.playerPickedName) {
          ...
          this.draftStarted =
            this.currentPickNumber !== 1 || this.currentRoundNumber !== 1;
        }
      });
      if (round.reverse) {
        round.picks.reverse();
  ...
  refreshData() {
    refreshApex(this.draftResult).then(() => {
      this.loading = false;
    });
  }
