...
const CLASSES =
  "slds-align_absolute-center slds-text-align_center slds-text-heading_large slds-m-around_xx-small slds-p-vertical_xx-small full-height";
...
export default class DraftMessages extends LightningElement {
  ...
  get showCountdown() {
    if (
      this.draftSettings?.Enable_Countdown__c &&
      this.draftSettings?.Draft_Start_Date__c
    ) {
  ...
  startCountdown() {
    ...
    // eslint-disable-next-line @lwc/lwc/no-async-operation -- interval is cleared in disconnectedCallback and once the countdown reaches zero
